...
| Mac mod 预编译包 | [下载录制器 ZIP](https://github.com/Jtshieh/STS2-Tools/releases/download/v0.1.1-alpha/Sts2Recorder-macos-arm64-v0.1.0-alpha.zip)，用于录制游玩，包含录制器 DLL、mod 描述和许可文件。 |
...
使用 Python 运行[合成决策示例](examples/README.zh-CN.md)，在配置游戏前查看观察、动作与后继。相同的读取器也可处理 Linux 导出的动作日志。
...
下载 [Mac mod ZIP](https://github.com/Jtshieh/STS2-Tools/releases/download/v0.1.1-alpha/Sts2Recorder-macos-arm64-v0.1.0-alpha.zip)，将 `Sts2Recorder.dll` 和 `Sts2Recorder.json` 放入 `SlayTheSpire2.app/Contents/MacOS/mods/`，启动游戏。主菜单会显示 `RECORDER READY` 和本次会话的日志目录。

按照 [Mac 录制与导出说明](mac/README.zh-CN.md) 取得轨迹，再[接入 Linux 回放](docs/REPLAY.zh-CN.md)，或将决策观察与人类选择接入自己的数据集处理流程。

## 参与贡献

报告问题、提交修改和选择相关检查的方法见[贡献说明](CONTRIBUTING.zh-CN.md)。
